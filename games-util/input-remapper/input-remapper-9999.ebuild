...
inherit distutils-r1 systemd udev desktop xdg git-r3
...
src_install() {
	...
	# The scripts above keep their "#!/usr/bin/env python3" shebang from
	# upstream, which resolves to whatever python3 happens to be the
	# system default -- not necessarily the interpreter this package was
	# actually built and installed for. Pin them to the right one.
	python_fix_shebang "${ED}/usr/bin/input-remapper-gtk"
	python_fix_shebang "${ED}/usr/bin/input-remapper-service"
	python_fix_shebang "${ED}/usr/bin/input-remapper-control"
	python_fix_shebang "${ED}/usr/bin/input-remapper-reader-service"
